...
.. code-block:: cql
   ...
   tablet_hints: `tablet_hint` [',' `tablet_hint`]
   
   tablet_hint: 'expected_data_size_in_gb' ':' <int>
...
.. list-table::
   ...
   * - ``tablet_hints``
     - map
     - see below
     - :ref:`Per-table tablet hints <cql-tablet-hints-options>`
...
.. _cql-tablet-hints-options:

Per-table tablet hints
######################
...
The following per-table ``tablet_hints`` options can be used to tune the tablet allocation logic for the table
if its data size, or performance requirements are known in advance.

=============================== =============== ===================================================================================
 Option                          Default         Description
=============================== =============== ===================================================================================
 ``expected_data_size_in_gb``   0               This option provides a hint for the number of tablets required for the anticipated
                                                table size (before replication).  It is calculated by dividing the expected data
 ...
 ``min_tablet_count``           0               Determines the minimum number of tablets to allocate for the table.
                                                ...
                                                of nodes and shards in the datacenter.  It is recommended to use higher-level hints
...
and the minimum tablet count calculated from the table's ``tablet_hints``, if any.
If multiple tablet hints are provided, ScyllaDB uses the maximum tablet count derived by each hint individually.
If the keyspace ``initial`` tablets is set to zero and no ``tablet_hints`` are provided,
...
to go below the minimum tablet count, or the per-shard tablet-count as per the above hints.
...
to drop the tablet hints or to adjust them respectively, to fit the new requirements.
