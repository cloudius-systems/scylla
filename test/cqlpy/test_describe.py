...
# This test compares the content of `system_schema.tables` and `system_schema.columns` tables
# when providing tablet_hints to CREATE TABLE.
def test_desc_table_with_tablet_hints(cql, test_keyspace, random_seed, has_tablets):
    if has_tablets:  # issue #18180
        global counter_table_chance
        counter_table_chance = 0
    tablet_hints = {
        'min_tablet_count': '100',
        'min_per_shard_tablet_count': '10.5',   # Verify that a floating point value works for this hint
        'expected_data_size_in_gb': '50',
    }
    with new_random_table(cql, test_keyspace, tablet_hints=tablet_hints) as tbl:
        desc = cql.execute(f"DESC TABLE {tbl}")
...
def new_random_table(cql, keyspace, udts=[], tablet_hints={}):
    ...
    if is_scylla(cql):
        ...
        if tablet_hints:
            extras["tablet_hints"] = str(tablet_hints)
