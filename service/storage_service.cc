...
future<> storage_service::raft_initialize_discovery_leader(const join_node_request_params& params) {
        ...
        const auto new_group0_state_id = utils::UUID_gen::get_random_time_UUID_from_micros(std::chrono::microseconds{ts});
        ...
        group0_command group0_cmd{.change{std::move(change)},
                .history_append{db::system_keyspace::make_group0_history_state_id_mutation(new_group0_state_id,
                        gc_clock::duration{std::chrono::duration_cast<gc_clock::duration>(std::chrono::weeks{1})},
                        "bootstrap: adding myself as the first node to the topology")},
                .prev_state_id{utils::UUID{}},
                .new_state_id{new_group0_state_id},
                .creator_addr{_sys_ks.local().local_db().get_token_metadata().get_topology().my_address()},
                .creator_id{params.host_id}};

        SCYLLA_ASSERT(std::holds_alternative<topology_change>(group0_cmd.change));
        auto chng = std::get<topology_change>(group0_cmd.change);
        co_await write_mutations_to_database(_qp.proxy(), group0_cmd.creator_addr, std::move(chng.mutations));
        canonical_mutation& history = group0_cmd.history_append;
        co_await _migration_manager.local().get_storage_proxy().mutate_locally({history.to_mutation(_db.local().find_schema(history.column_family_id()))}, nullptr);
}
